...
Set `category: experience` on research, work, startup and professional-experience photos; use `category: scenery` for landscapes and leisure photos. Set the category on each desktop group too. **Unseen content takes priority, with categories mixed throughout the round.** Available category counts and viewed progress determine spacing, with random variation so work photos are not exhausted early and followed by a long scenic tail. Photos within each category are shuffled too. `2:1` gives experience a small earlier-position preference, not twice the probability or playback count; complete no-repeat rounds and even interleaving take priority. Adjust these two numbers to change the preference. Set a category to `0` to disable it; setting both to `0` hides the banner.

All enabled photos/groups eligible at the current viewport finish before a new global round begins; width limits still apply. `localStorage` preserves viewed progress across reloads, which reshuffle only the unseen content. This is best effort within the same browser, device and site address; without storage, progress lasts only for the current page. Groups share viewing history with their member photos, so layout changes prioritize remaining unseen content. The “Next” caption always names the queued selection; hovering and language changes do not reshuffle it. Startup waits for the actual random photo instead of flashing a fixed old poster; a static fallback remains for visitors without JavaScript. Selection weights do not change the timing above.
