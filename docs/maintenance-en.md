...
Set `category: experience` on research, work, startup and professional-experience photos; use `category: scenery` for landscapes and leisure photos. Set the category on each desktop group too. **Unseen content takes priority; weights then choose between categories.** When both categories have unseen photos, `2:1` makes experience twice as likely for the next selection. Once one category is exhausted, the other finishes before anything repeats. Weights therefore influence order, not a fixed long-term playback ratio. Change these two numbers to adjust the preference. Set a category to `0` to disable it; setting both to `0` hides the banner.
