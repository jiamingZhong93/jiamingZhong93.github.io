...
[Repository overview](../README.md) · [中文维护指南](README-zh.md) · [Live homepage](https://jiamingzhong.world/)

**Daily workflow:** preview locally → edit English and Chinese → check desktop and phone → commit and push → confirm deployment.

[Preview](#1-start-a-local-preview) · [Content and translations](#2-find-the-content-to-edit) · [Research and images](#3-update-research-projects-and-pictures) · [Publications](#4-update-publications) · [Publishing](#5-publish-changes) · [Taking the site offline](#6-take-offline-or-restore)
...
| Browser title and bilingual search description | [_config.yml](../_config.yml): `title`, `author.name_zh`, `description/description_zh`; rendered by [_includes/seo.html](../_includes/seo.html) |
| JZ browser icon | [images/favicon.svg](../images/favicon.svg), with PNG/ICO exports in `images/` |
...
Set `category: experience` on research, work, startup and professional-experience photos; use `category: scenery` for landscapes and leisure photos. Set the category on each desktop group too. **`1.5:1` gives the experience category about 60% of displays and scenery about 40%**, independent of how many photos each category contains. Categories are interleaved with random variation; each category shuffles and finishes its own eligible photos before repeating. A desktop group counts as one display. Adjust the numbers to change the frequency; set a category to `0` to disable it, or both to `0` to hide the banner. If only one category is eligible, it fills the available slots.

`localStorage` preserves each category's viewed progress and the selection balance across reloads. Refreshing reshuffles the remaining photos without restarting the frequency balance; width limits still apply. This is best effort within the same browser, device and site address; without storage, progress lasts only for the current page. Groups share viewing history with their member photos, so layout changes prioritize remaining unseen content within each category. Consecutive slides avoid the same scene whenever another is available. The “Next” caption always names the queued selection; hovering and language changes do not reshuffle it. Startup waits for the actual random photo instead of flashing a fixed old poster; a static fallback remains for visitors without JavaScript. Selection weights do not change the timing above.
...
- Optional single-photo `subject: [x, y, width, height]` with `source_size: [width, height]` protects a subject rectangle in source pixels, including a little breathing room. For ordinary `cover` photos (without `crop` or `mobile_crop`), it automatically centers that region instead of using `position` / `mobile_position`. If the whole region cannot fit the current banner, that photo is skipped for that screen size. This keeps geese and tall landmarks intact without stretching, sidebars, or a taller banner. Update or remove both fields when replacing the source. It is not inherited by desktop groups.
- Optional per-image `desktop_max_width` limits desktop display to that window width in pixels. Omit it to allow all desktop widths; F1TENTH has no width limit.
...
- Use `crop` (x, y, width, height in source pixels) with `source_size: [width, height]` to trim empty borders without editing the file. Desktop `fit` then frames that crop: `contain` preserves it completely; `cover` fills the banner. Phones always fill the banner with `cover`. Cropped frames are centered, so adjust `crop` rather than `position` to move the subject. For a separate phone/tablet crop (window widths up to 800 px), add `mobile_crop: "x y width height"`; it uses the same `source_size` and falls back to `crop` when omitted. F1TENTH uses this to frame the car more closely on phones. Update or remove the crop fields and `source_size` when replacing the source photo.
...
The editable JZ mark is [images/favicon.svg](../images/favicon.svg). If you redesign it, keep these matching exports in `images/`: `favicon.ico`, `favicon-16x16.png`, `favicon-32x32.png`, `apple-touch-icon.png` (180 × 180), `android-chrome-192x192.png` and `android-chrome-512x512.png`. SVG changes do not automatically regenerate the PNG/ICO files.

After replacing the icons, update their shared version suffix (currently `?v=jz-1`) in [_includes/head/custom.html](../_includes/head/custom.html) and [images/site.webmanifest](../images/site.webmanifest) so browsers fetch the new files.
