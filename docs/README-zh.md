...
研究、工作、创业及职业经历照片填写 `category: experience`，风景与休闲照片填写 `category: scenery`；电脑组合也单独填写 `category`。权重针对**整个类别，而非每张照片**：两类都有可用照片时，`2:1` 表示经历类约占三分之二，不会因新增大量风景照而被稀释。日后只需调整这两个数字；某类设为 `0` 可暂停该类，两类都为 `0` 则隐藏横幅。

每个类别内部随机排序，同类照片或组合看完一轮后再开始下一轮，尽量避免重复。浏览器通过 `localStorage` 保留已看记录，刷新后继续尽量避开已看内容；记录仅限同一浏览器和设备，存储不可用时只在当前页面内有效。“下一张”显示的是已选定的真实下一张，悬停及切换语言不会重新抽取。权重不影响上述播放时间。
