...
研究、工作、创业及职业经历照片填写 `category: experience`，风景与休闲照片填写 `category: scenery`；电脑组合也单独填写 `category`。**`1.5:1` 表示经历类约占播放次数的 60%，风景类约占 40%**，不受两类照片数量影响。类别之间随机穿插，每类独立随机排列，播完该类当前可用的照片后再开始下一轮；电脑组合按一次播放计算。修改这两个数字可调整出现频率；某类设为 `0` 可暂停该类，两类都为 `0` 则隐藏横幅。当前尺寸只有一类可用时，就播放该类。

浏览器通过 `localStorage` 保留各类别的已看记录和播放比重进度，刷新只重新排列剩余照片，不重置类别分配；屏幕宽度限制仍然生效。记录仅限同一浏览器、设备和网站地址，存储不可用时只在当前页面内有效。组合与其成员共享观看记录，切换屏幕布局时优先补齐该类未见内容。有其他可用内容时，相邻两张避免重复同一场景。“下一张”显示的是已选定的真实下一张，悬停及切换语言不会重新抽取。刷新时等待随机首张就绪，不先闪现一张固定旧图；禁用 JavaScript 时才显示静态备用图。权重不影响上述播放时间。
...
- `fit`：电脑端使用默认的 `cover`，通过裁切铺满横幅，不拉伸。`contain` 是旧版兼容选项，完整显示照片但会留下虚化补边。F1TENTH 使用 `cover`，没有模糊补边。避免大面积虚化：优先重新取景、加入相关研究照片组合，或设为 `desktop: false`。手机端强制使用 `cover`；旧字段 `mobile_fit` 可省略，不能再开启模糊补边。手机布局适用于不超过 800 px 的窗口宽度。
...
- 单张照片可选填 `subject: [x, y, 宽, 高]`，配合 `source_size: [原图宽, 原图高]`，以原图像素标出需要完整保留的主体，建议留少量余量。它用于没有 `crop` 或 `mobile_crop` 的普通 `cover` 照片，自动居中取景，替代 `position` / `mobile_position`。当主体区域无法完整放入当前横幅时，仅在该屏幕尺寸下跳过这张照片，避免截掉鹅头或塔尖，也不拉伸、不补边、不增加横幅高度。换图后同步更新或删除这两个字段；电脑拼接组不会继承该设置。
- 单张照片可选填 `desktop_max_width`，限制电脑端显示它的最大窗口宽度（像素）；省略则适用于所有电脑宽度。F1TENTH 不设宽度限制。
...
- 使用 `crop`（原图像素中的 x、y、宽、高）和 `source_size: [宽, 高]`，可以裁掉多余留白而不修改原文件。电脑 `fit` 再决定如何显示裁切区域：`contain` 完整保留，`cover` 铺满横幅；手机始终用 `cover` 铺满。裁切区域居中显示，要移动主体请调整 `crop`，此时 `position` 不控制裁切区域。如需手机和平板（窗口不超过 800 px）单独取景，可添加 `mobile_crop: "x y 宽 高"`，沿用同一 `source_size`；省略时沿用 `crop`。F1TENTH 使用这一字段在手机上突出小车。替换原照片时同步更新或删除裁切字段与 `source_size`。
