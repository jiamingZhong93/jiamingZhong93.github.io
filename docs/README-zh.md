...
研究、工作、创业及职业经历照片填写 `category: experience`，风景与休闲照片填写 `category: scenery`；电脑组合也单独填写 `category`。**先保证未看过的照片优先，再让两类内容穿插覆盖整轮**：根据每类可用数量和已播进度分配出场位置，并加入随机变化，避免先播完工作照、后面只剩一长段风景。类别内的照片也随机排序。`2:1` 仅给予经历类轻微的前置偏好，并不表示播放概率或次数为两倍；整轮不重复和均匀穿插优先。日后可调整这两个数字；某类设为 `0` 可暂停该类，两类都为 `0` 则隐藏横幅。
...
- 单张照片可选填 `subject: [x, y, 宽, 高]`，配合 `source_size: [原图宽, 原图高]`，以原图像素标出需要完整保留的主体，建议留少量余量。它用于没有 `crop` 的普通 `cover` 照片，自动居中取景，替代 `position` / `mobile_position`。当主体区域无法完整放入当前横幅时，仅在该屏幕尺寸下跳过这张照片，避免截掉鹅头或塔尖，也不拉伸、不补边、不增加横幅高度。换图后同步更新或删除这两个字段；电脑拼接组不会继承该设置。
